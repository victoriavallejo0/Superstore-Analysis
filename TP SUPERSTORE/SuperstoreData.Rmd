...
El presente documento tiene como objetivo el establecimiento de precios y cantidades para la maximización de los ingresos totales del SuperStore. Esto se podrá lograr a través del cumplimiento de los siguientes sub-objetivos:
  ...
  c. Metodología de promociones para la maximización del ingreso.
...
library(funModeling)
library(dplyr)
df_seleccionada= df%>%select(Category,Segment,Sales,Price, Quantity, Discount, `Product Name`)
...
```{r echo=FALSE}
independecia_variables = table (df$Segment,df$Category)
...
Se indentifica cuales son los productos por categoría que se acercan al precio maximizador.
CATEGORÍA OFFICE SUPPLIES
```{r}
...
```{r echo=FALSE}
df_furniture=df%>%filter(Category=="Furniture")
df_Office=df%>%filter(Category=="Office Supplies")
df_Technology=df%>%filter(Category=="Technology")
```
...
```
Elasticidad = 0.02449035 
Representa el cambio porcentual en la cantidad demandada (-2,45%) causado por un 1% de aumento en el precio.
...
```{r echo=FALSE}
e_furniture=lm(log(df_furniture$Quantity) ~log(df_furniture$Price))
e_Office=lm(log(df_Office$Quantity) ~log(df_Office$Price))
e_Technology=lm(log(df_Technology$Quantity) ~log(df_Technology$Price))
```

FURNITURE
```{r echo=FALSE}
summary(e_furniture)
e_furniture$coefficients
```
...
TECHNOLOGY
```{r echo=FALSE}
summary(e_Technology)
e_Technology$coefficients
```
...
df=df %>% mutate(Discount1=case_when
              (Category=="Furniture"~0.9,
                Category=="Technology"~0.8,
                Category=="Office Supplies"~1.3))
...
df=df %>% mutate(Discount2=case_when
              (Category=="Furniture"~1.1,
                Category=="Technology"~1.2,
                Category=="Office Supplies"~0.85))
...
df=df %>% mutate(Discount3=case_when
              (Category=="Furniture"~0.9,
                Category=="Technology"~1.1,
                Category=="Office Supplies"~0.9))

df %>% select(c('Price','Quantity', "Category", "Discount3")) %>% ggplot(aes(x = log(Price*Discount3), y = Quantity,color=Category))  + geom_smooth(method = lm)
```
...
log2= log10(Pf*1.1)
ITf2 = (Pf*1.1) * 4.1
"Ingreso con descuento 2"
ITf2

Pfd3= Pf*0.9
"Precio con descuento 3"
Pfd3
log3= log10(Pf*0.9)
ITf3 = (Pf*0.9) * 4.1
"Ingreso con descuento 3"
ITf3
```
```{r echo=FALSE}
#TECHNOLOGY
...
P2=ITf2+ITt2+ITo2
P3=ITf3+ITt3+ITo3
"Ingreso tot SIN promo"
P0
"Ingreso tot SIN promo 1"
P1
"Ingreso tot SIN promo 2"
P2
"Ingreso tot promo 3"
P3
"Diferencia promo 2 y 1"
P2-P1
"Diferencia promo 2 y 3"
P2-P3
"Diferencia promo 2 y sin promo "
P2-P0
```
La promoción que genera el mayor ingreso es la promoción 2, la cual concuerda con la elasticidad por Categoría. Al disminuir el precio de Office Supplies, la más elástica aumenta su cantidad demandada. 
El ingreso con la promoción 2 supera el ingreso actual de la base. 
